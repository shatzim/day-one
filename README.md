# day-one — a budgeting application
...
Clone the day-one repository using [git][git]:

```
git clone https://github.com/shatzim/day-one.git
cd day-one
```
...
*Note that bower components are located in `app/components` folder as configured in the `.bowerrc` file.  
...
* the configuration is found at `test/unit/karma.conf.js`
